...
public abstract class GoogleApiClientRequestSingle<T, R extends Result>
    ...
    protected void onSingleClientConnected(GoogleApiClient googleApiClient) {
        createRequest(googleApiClient)
                .setResultCallback(new ResultCallbacks<Result>() {
                    @Override
                    public void onSuccess(@NonNull Result result) {
                        GoogleApiClientRequestSingle.this.onSuccess(unwrap((R) result));
                    }

                    @Override
                    public void onFailure(@NonNull Status status) {
                        onError(new ClientException("Error in client request. " + status
                                .getStatusMessage()));
                    }
                });
    }
